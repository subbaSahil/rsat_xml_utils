...
Interactions.wait_and_click(driver, By.XPATH, "//a[@data-dyn-title='Released product maintenance']")
time.sleep(1)
Interactions.wait_and_click(driver, By.XPATH, "//li[contains(@data-dyn-controlname,'ReleasedProductsRecentlyCreated')]")
# Inputting into: InventTable_ItemId
if(Interactions.check_input_ancestor_is_table(driver, By.XPATH, "//input[contains(@name,'InventTable_ItemId')]") or Interactions.check_input_ancestor_is_table(driver, By.XPATH, "//input[contains(@aria-label,'Item number')]") ):
    #clicking inside grid: InventTable_ItemId
    if(Interactions.check_element_exist(driver, By.XPATH, "(//input[contains(@name,'InventTable_ItemId')])[1]")):
         Interactions.wait_and_send_keys(driver, By.XPATH, "(//input[contains(@name,'InventTable_ItemId')])[1]", "")
    ...
    if(Interactions.check_element_exist(driver, By.XPATH, "//input[contains(@name,'InventTable_ItemId')]")):
         Interactions.wait_and_send_keys(driver, By.XPATH, "//input[contains(@name,'InventTable_ItemId')]", "")
...
# Clicking button: GridConvertible
user_input = input("Press data to select: ")
Interactions.scroll_and_click_row(driver, By.XPATH, "//div[contains(@class,'fixedDataTableRowLayout_')]/ancestor::div[@role='grid']", f"//input[@value='{user_input}']/ancestor::div[@class='fixedDataTableRowLayout_body']/div[1]//div[@role='checkbox']")
Interactions.press_enter(driver, By.XPATH, "//input[@value='"+user_input+"']")
# Clicking button: GridConvertible
