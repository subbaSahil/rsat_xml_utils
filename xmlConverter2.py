    ...
    for index, action in enumerate(user_actions):
        ...
        if node is not None:
            ...
            if control_name_elem is not None and control_name_elem.text:
                control_names.append(control_name_elem.text)
            ...
            # Check if <Arguments> has no children
            arguments_elem = node.find(".//Arguments", namespaces=namespaces)
            if control_type == "Grid" and (arguments_elem is None or len(arguments_elem) == 0):
                continue  # Skip this node if it's Grid with no arguments

        # Store in dictionary
        if ref not in actions_dict:
            actions_dict[ref] = {
                "Description": description,
                "ControlNames": set(control_names),
                "ControlLabel": control_label,
                "ControlType": control_type,
                "Value": value,
                "NavigationPath": navigation_path,
                "Annotations": annotations_elem,
